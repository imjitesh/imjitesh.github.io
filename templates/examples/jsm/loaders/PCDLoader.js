...
PCDLoader.prototype = {
	...
	parse: function ( data, url ) {
		...
		function parseHeader( data ) {
			...
			if(result1 == -1){PCDheader.data = 'binary_uncompressed'; return PCDheader}
		...
		if(url.indexOf('.bin') == -1){
			...
			var textData = new Float32Array( data );
			var PCDheader = {};
			PCDheader['data'] = 'binary_uncompressed';
		}
		...
		if(PCDheader.data === 'binary_uncompressed'){
			for (var row = 0; row < textData.length; row = row + 4) {
				position.push(textData[row]);
				position.push(textData[row+1]);
				position.push(textData[row+2]);
      			}
      		}
